---
title: Loanback to Fishbone Commercial Properties Ltd (secured on 145 High Street East, Wallsend)
category: Assets
status: active
sensitive: false
created: 2026-09-06
updated: 2026-09-07
sources:
  - ../../Raw/Legal Mortgage 29 April 2025.pdf
  - ../../Raw/Completion Statement.pdf
  - ../../Raw/Receipted Invoice.pdf
  - https://mail.google.com/mail/u/0/#all/1963deba41414b6b (Gmail: Signable confirmation, loan agreement signed, 16/04/2025)
  - https://mail.google.com/mail/u/0/#all/195ed15bd4149079 (Gmail: Empowered Lending, rate and default rate requested, 31/03 to 01/04/2025)
  - https://mail.google.com/mail/u/0/#all/19614f0e4ba798b9 (Gmail: Dollman & Pritchard instructed, 07/04 to 16/04/2025)
  - https://mail.google.com/mail/u/0/#all/19681349080e39a1 (Gmail: Dollman & Pritchard completion 29/04/2025 and Companies House registration 08/05/2025)
  - https://mail.google.com/mail/u/0/#all/19bfa3258b50f266 (Gmail: Dollman & Pritchard, HM Land Registry registration completed, 26/01/2026)
  - https://mail.google.com/mail/u/0/#all/19b2bebb75994919 (Gmail: bookkeeper to RMT Accountants with the repayment schedule attached, 17/12/2025)
  - https://mail.google.com/mail/u/0/#all/195f547bc88cc4d4 (Gmail: Empowered Pensions, Deed of Adherence for the borrower, 02/04 to 14/04/2025)
related:
  - ../Suppliers/dollman-pritchard.md
  - ../People/m-gaudiesius.md
  - ../People/a-prutkovas.md
  - ../People/i-fedonina.md
  - ../People/empowered-trustees.md
  - ../Finance/hmrc-registration.md
  - ../Processes/scheme-rules.md
  - ../Suppliers/empowered-pensions.md
  - ../Suppliers/metro-bank.md
  - ../Finance/transfers-in.md
  - ../Employers/fishbone-properties-ltd.md
  - ../Finance/scheme-accounts-and-returns.md
---

# Loanback to Fishbone Commercial Properties Ltd (secured on 145 High Street East, Wallsend)

The scheme's one known investment: a loan of 41,500 to Fishbone Commercial Properties Ltd, a
group company, secured by a first legal mortgage over 145 High Street East, Wallsend. The
security deed, the solicitors' completion statement and their receipted invoice are on file. **The
loan agreement itself, dated 16 April 2025, is not**, so the rate, term and repayment schedule
below still rest on the group's Loans wiki and are `(unverified)`. A signed copy of the agreement,
the heads of terms, the Companies House registration certificate, the Land Registry official copy
and a repayment schedule all exist **as email attachments in the owner's Gmail** (found
2026-09-06; the tooling cannot download attachments, so a person must save them into `Raw/`).

Asset Register id: none yet. When issued it will be `FSS 0001` with `Asset class` "Loanback
receivable", `Loan type` "SSAS loanback", `Counterparty` Fishbone Commercial Properties Ltd.

## Key facts

| Item | Value | Source |
|---|---|---|
| Lender | Mindaugas Gaudiesius, Irina Fedonina, Andrejus Prutkovas and Empowered Trustees Ltd (12291059), as Trustees of the Fishbone SSAS | [^1] p.3 |
| Borrower | Fishbone Commercial Properties Ltd, company number 13687238, registered office 6 Beverley Place, Wallsend NE28 7BH | [^1] p.3 |
| Loan amount | 41,500.00 | [^2] |
| Costs deducted at completion | 1,352.20 | [^2] |
| Net advance to the borrower | 40,147.80 | [^2] |
| Loan agreement | dated 16 April 2025 (day handwritten), "for the provision of the loan facilities secured by this deed"; **not on file** | [^1] p.6 |
| Security deed | Legal Mortgage dated 29 April 2025, 44 numbered pages plus cover | [^1] cover, p.3 |
| Security | first legal mortgage over "all that freehold property known as 145 High Street East, Wallsend, Tyne and Wear NE28 7RL forming part only of the land registered at HM Land Registry under title number TY59507, as shown coloured blue on the attached plan"; fixed charges over insurance policies, contracts and licences; assignment of insurance proceeds | [^1] p.9 cl. 3, p.39 Sch. 1 |
| Maximum Amount | "50% of the value of the Pension Scheme"; borrowing from the Lender may never exceed it | [^1] p.6, p.10 cl. 4.2 |
| Land Registry restriction | Form P restriction consented to, in favour of the four trustees | [^1] p.10 cl. 4.1 |
| Solicitors (for the trustees) | Dollman & Pritchard, Caterham, ref AJA/FIS53.1 | [^1] cover, [^3] |
| Solicitors' invoice | no. 33194, tax point 16/04/2025, total 1,244.20, marked received with thanks | [^3] |
| Execution | signed as a deed by each of the three member trustees before a witness, by Empowered Trustees Ltd acting by its director Lars Ian Singleton, and by the borrower acting by its two directors M Gaudiesius and A Prutkovas | [^1] pp.40-44 |
| Loan process | run by Empowered Lending Ltd (CRN 14029489), reference LA01801: webform 17/02/2025, application 17/03/2025, loan process fee invoiced and paid 24/03/2025 | [^4] `(email)` |
| Rate and default rate | the owner asked on 01/04/2025 to "amend the loan interest rate to 5.5% and set the default rate to 8.25%", with capital repaid monthly; heads of terms signed through Signable 07/04/2025 | [^5] `(email; the signed heads of terms are an attachment, not on file)` |
| Loan agreement signed | Signable "Document signed confirmation" of 16/04/2025 with the signed `FishboneSSAS_Loanback(£41.5k)_LA01801_2025-04-xx.pdf` attached | [^6] `(email; attachment not on file)` |
| Borrower's status with the scheme | Empowered Pensions sent a Deed of Adherence for Fishbone Commercial Properties Ltd on 09/04/2025 (fee 250 plus VAT, wet signature) and on 14/04/2025 said the company was already an adhered company | [^7] `(email; deed not on file)` |
| Charge registered at Companies House | Dollman & Pritchard emailed the registration certificate on 08/05/2025 | [^8] `(email; certificate not on file)` |
| Restriction registered at HM Land Registry | "REGISTRATION COMPLETED" from Dollman & Pritchard 26/01/2026 with the Official Copy of the Register, Title Plan and Completion Sheet for TY59507 attached; a 25.00 balance was returned to the borrower 30/01/2026 | [^9] `(email; official copies not on file)` |
| Repayment schedule | `Fishbone Loanback Repayment Schedule.pdf` attached to the group bookkeeper's email to RMT Accountants of 17/12/2025; RMT had understood the loan to be 40,147.80 (the net advance), which the trustees' own figure of 41,500 contradicts | [^10] `(email; schedule not on file)` |

Terms from the group's Loans wiki on Drive (facility page 21/08/2026, reference `K0555`), **none
of them evidenced by a document in this knowledge base** `(unverified)`: interest 5.5 percent
tracking base rate; 60 months; 790.13 a month; final payment 28/04/2030; balance 31,495.87 as at
21/08/2026. The 5.5 percent rate matches the rate the owner asked the lender's administrator to
write into the agreement on 01/04/2025 [^5]; whether the agreement tracks base rate or fixes the
rate is not stated in the email. The Fishbone Commercial Properties Ltd knowledge base records the same loan from the
borrower's side (accounts to 30/04/2025 showing a secured loan of 41,500; bank statements showing
790.13 leaving around the 29th of each month); link there, do not copy.

## Details

**Timeline from the documents.** The solicitors' invoice has a tax point of 16/04/2025 [^3]; the
loan agreement is dated 16 April 2025 [^1] p.6; the mortgage deed is dated 29 April 2025 [^1]
cover; the completion statement is undated but carries the same matter reference [^2]. So the
money moved on or about 29/04/2025, which fits the borrower's monthly payment date of about the
29th recorded in the sister knowledge base and the final payment date of 28/04/2030 in the Loans
wiki.

**What the completion statement shows.** From the 41,500 loan the solicitors deducted their fee
of 950 plus VAT, a bank transfer fee, company, winding-up, Land Registry and land charges
searches, anti-money-laundering identity checks for three names, office copy entries, a Companies
House filing fee of 15.00 and a Land Registration fee of 45.00, total 1,352.20, and advanced
40,147.80 to the borrower [^2]. The receipted invoice covers 1,244.20 of that; the balance of
108.00 is the third identity check plus the registration and search fees that appear only on the
completion statement [^2] [^3]. The borrower therefore received 40,147.80 net but owes the full
41,500 `(inference from the statement, not stated in it)`.

**Charge registration.** The Companies House filing fee and the Land Registration fee were paid
[^2]. ~~Neither registration is evidenced on file.~~ Corrected 2026-09-06 from Gmail: the
solicitors emailed the Companies House registration certificate on 08/05/2025 [^8], within the
21-day window from 29/04/2025, and on 26/01/2026 reported the Land Registry registration completed,
attaching the official copy, title plan and completion sheet for TY59507 [^9]. The documents
themselves are still not in `Raw/`, so the registrations are evidenced by correspondence only
`(email)`. The solicitors' emails describe the matter as a "Blanket Charge: 145 High Street East
and 2 & 2A Ferndale Avenue", the whole of title TY59507; the owner confirmed on 08/04/2025 that
the charge was to be over 145 High Street East only, and the executed deed charges that part
[^1] Sch. 1, [^11].

**The deed.** The trustees are the Lender. The Secured Liabilities are everything owed under the
Loan Agreement and any other loan agreement between the same parties [^1] p.6. Events of default
include non-payment, breach, insolvency and a value-of-assets test [^1] pp.4-5. The Lender's
maximum exposure is tied to the scheme's own value, not to a fixed sum [^1] p.6. Clauses 6.11 to
29 (representations, general and property covenants, powers of the Lender, enforcement,
receivers, costs, release, notices; PDF pages 14 to 39) were not summarised in this session. The
plan referred to in Schedule 1 is not part of the scanned copy.

**Both sides of the table.** The two directors who signed for the borrower are two of the four
trustees who signed for the lender [^1] pp.40, 42, 44. The scheme rules allow a trustee to act
despite a personal interest (rule 6.6) and require the corporate trustee's written agreement to
exercise the investment and lending powers (rule 6.3.1); Empowered Trustees Ltd executed the deed
[^1] p.43. See [Scheme rules](../Processes/scheme-rules.md).

**HMRC authorised employer loan conditions**, as far as the documents on file allow:

| Condition | Evidence on file | Status |
|---|---|---|
| Secured by a first charge over assets of at least equal value | First legal mortgage executed 29/04/2025 [^1]; no valuation on file; the FCP knowledge base flags that the March 2025 valuation assumed structural repairs still in progress | charge: verified; value: `(unverified)` |
| Amount no more than 50 percent of the scheme's net assets | The deed's Maximum Amount is 50 percent of the scheme's value [^1] p.6; the scheme's value at 16/04/2025 is not on file. The transfer values quoted for the three members' transfers in, before contributions or growth, put the loan at about 39 to 41 percent, **but the third member's Aviva transfer had not completed by July 2025 and her Nest transfer is not evidenced as completed**, so at April 2025 the scheme most likely held only the two directors' transfers, against which 41,500 is about 49.8 percent; see [Transfers in](../Finance/transfers-in.md) | `(unverified: quoted values, not receipts)` **and close to the limit** |
| Interest at least 1 percent above base rate | Loans wiki: 5.5 percent tracking base rate; the owner asked for 5.5 percent and a default rate of 8.25 percent on 01/04/2025 [^5] | `(unverified: agreement not on file)` |
| Term no more than five years | Loans wiki: 60 months, final payment 28/04/2030 | `(unverified)` |
| Equal instalments of capital and interest | Loans wiki: 790.13 a month | `(unverified)` |

## Open questions

- The **loan agreement dated 16 April 2025**: principal, rate, term, repayment schedule, default
  interest. ~~Dollman & Pritchard (ref AJA/FIS53.1) or Empowered Pensions Ltd should hold it.~~
  The signed copy is attached to the Signable email of 16/04/2025 [^6]; a person needs to save it
  into `Raw/`, with the heads of terms [^5] and the repayment schedule [^10].
- ~~Evidence that the charge was registered at Companies House against company 13687238 (form
  MR01, within 21 days of 29/04/2025) and that the Form P restriction was entered on title TY59507.~~
  Both evidenced by the solicitors' emails of 08/05/2025 and 26/01/2026 [^8] [^9]; the certificate
  and the official copies need saving into `Raw/`.
- **The 50 percent test at April 2025 looks tight** (about 49.8 percent of the two directors'
  quoted transfer values, before contributions, growth, fees and the third member's later
  transfers). The scheme's actual value at 16/04/2025, from the Metro Bank balance or the
  administrator's loan file, would settle it.
- RMT Accountants understood the loan to be 40,147.80; the borrower's accounts should carry 41,500.
- The valuation of 145 High Street East relied on for the loan, and whether the repairs it
  assumed are complete.
- The scheme's value at 16/04/2025, to test the 50 percent limit.
- The repayment record from the scheme's Metro Bank statements (none on file).
- Whether the loan was reported to HMRC where an Event Report requires it.

## Changes

| Date | Change | Change-log ref |
|---|---|---|
| 2026-09-06 | Created from the three Raw items in the front matter; first article in `Assets/` | `Outputs/change-log-2026-09-06-loan-security-and-governing-documents.md` |
| 2026-09-06 | 50 percent test estimated from the transfer values | `Outputs/change-log-2026-09-06-ssas-folder-extraction.md` |
| 2026-09-06 | Loan process, agreement signing, charge registration at Companies House and the Land Registry, repayment schedule and borrower adherence found in Gmail; 50 percent test recomputed for April 2025 | `Outputs/change-log-2026-09-06-ssas-folder-extraction.md` |
| 2026-09-07 | Reciprocal `related:` links repaired | `Outputs/change-log-2026-09-07-email-outstanding-items-addendum.md` |

## Sources

[^1]: [Legal Mortgage dated 29 April 2025, Trustees of the Fishbone SSAS and Fishbone Commercial Properties Ltd, scanned, 45 PDF pages](<../../Raw/Legal Mortgage 29 April 2025.pdf>) - cover; contents; parties and definitions pp.3-8; clauses 2-6.10 pp.9-13; Schedule 1 p.39; execution pp.40-44
[^2]: [Completion Statement, Dollman & Pritchard ref AJA/FIS53.1, 1 page, undated](<../../Raw/Completion Statement.pdf>)
[^3]: [Receipted invoice no. 33194, Dollman & Pritchard, tax point 16/04/2025, 1 page](<../../Raw/Receipted Invoice.pdf>)
[^4]: [Gmail: "Fishbone SSAS- Loanback Webform" 17/02/2025 and "Loanback Application for Fishbone SSAS ... for £ 41,500.00" 17/03 to 24/03/2025, Empowered Lending](https://mail.google.com/mail/u/0/#all/195a37639d72c0e2) `(email)`
[^5]: [Gmail: "RE: Loanback Application ... LA01801", owner to Empowered Lending, 31/03 and 01/04/2025; Signable confirmation of the heads of terms 07/04/2025](https://mail.google.com/mail/u/0/#all/195ed15bd4149079) `(email; attachment not on file)`
[^6]: [Gmail: "Document signed confirmation", Signable, 16/04/2025, with the signed loan agreement attached](https://mail.google.com/mail/u/0/#all/1963deba41414b6b) `(email; attachment not on file)`
[^7]: [Gmail: "Fishbone SSAS", Empowered Pensions administrator, 02/04 to 14/04/2025, Deed of Adherence for Fishbone Commercial Properties Ltd](https://mail.google.com/mail/u/0/#all/195f547bc88cc4d4) `(email; deed not on file)`
[^8]: [Gmail: "Loanback Application ... LA01801", Dollman & Pritchard, completion 29/04/2025 and Companies House registration certificate 08/05/2025](https://mail.google.com/mail/u/0/#all/19681349080e39a1) `(email; certificate not on file)`
[^9]: [Gmail: "REGISTRATION COMPLETED ...", Dollman & Pritchard, 26/01/2026, and "BALANCE TO BORROWER", 30/01/2026](https://mail.google.com/mail/u/0/#all/19bfa3258b50f266) `(email; official copies not on file)`
[^10]: [Gmail: "Re: Fishbone Commercial Properties Ltd - SSAS Loan", bookkeeper to RMT Accountants, 17/12/2025, repayment schedule attached](https://mail.google.com/mail/u/0/#all/19b2bebb75994919) `(email; schedule not on file)`
[^11]: [Gmail: "RE: Loanback Application ... LA01801", Dollman & Pritchard and owner, 07/04 to 16/04/2025: title TY59507 covers 145 High Street East and 2 & 2A Ferndale Avenue; charge over 145 High Street East only](https://mail.google.com/mail/u/0/#all/19614f0e4ba798b9) `(email)`
